			...
			<div class="um-admin-card-body">
				...
				<div class="um-my-account-order-viewer-sub-settings" style="margin-left: 24px; padding-left: 16px; border-left: 2px solid #dcdcde;">
					...
					<div class="um-form-field" id="um-my-account-admin-order-additional-meta-field" style="<?php echo empty($settings['my_account_admin_order_viewer_enabled']) ? 'display:none;' : ''; ?>">
						...
						<p class="description"><?php esc_html_e('Format: meta_field:Label[::flags]. Renders under each Order detail view. Optional flags: text_line_count, text-file-line-count, line_count, count_lines, preview, preview_file, file_preview, preview-modal, preview_modal, display_when_empty, display-empty, show_empty, show_if_empty, render_if_empty, fcf_file, fcf_file_upload, flexible_checkout_fields_file_upload. Separate values using commas and/or line breaks.', 'user-manager'); ?></p>
						<p class="description"><strong><?php esc_html_e('Render this as a Flexible Checkout Fields PRO File Upload Field (checkbox-style flag):', 'user-manager'); ?></strong> <?php esc_html_e('add the fcf_file flag to treat the stored meta value as a Flexible Checkout Fields PRO upload hash. The plugin looks up the file inside wp-content/uploads/woocommerce_uploads/flexible-checkout-fields/<hash>/ and uses that resolved location when linking, previewing, and counting lines.', 'user-manager'); ?></p>
						<p class="description"><?php esc_html_e('Example (FCF PRO File Upload with line count, preview, and always-show row): _volunteer_file:Volunteer File::fcf_file|text_line_count|preview|display_when_empty', 'user-manager'); ?></p>
					...
					<div class="um-form-field" id="um-my-account-admin-order-additional-meta-list-field" style="<?php echo empty($settings['my_account_admin_order_viewer_enabled']) ? 'display:none;' : ''; ?>">
						...
						<p class="description"><?php esc_html_e('Format: meta_field:Label[::flags]. Renders in Admin: Orders list (inline under action buttons). Optional flags: text_line_count, text-file-line-count, line_count, count_lines, preview, preview_file, file_preview, preview-modal, preview_modal, display_when_empty, display-empty, show_empty, show_if_empty, render_if_empty, fcf_file, fcf_file_upload, flexible_checkout_fields_file_upload. Separate values using commas and/or line breaks.', 'user-manager'); ?></p>
						<p class="description"><strong><?php esc_html_e('Render this as a Flexible Checkout Fields PRO File Upload Field (checkbox-style flag):', 'user-manager'); ?></strong> <?php esc_html_e('add the fcf_file flag to treat the stored meta value as a Flexible Checkout Fields PRO upload hash. The plugin looks up the file inside wp-content/uploads/woocommerce_uploads/flexible-checkout-fields/<hash>/ and uses that resolved location when linking, previewing, and counting lines.', 'user-manager'); ?></p>
						<p class="description"><?php esc_html_e('Example (FCF PRO File Upload with line count, preview, and always-show row): _volunteer_file:Volunteer File::fcf_file|text_line_count|preview|display_when_empty', 'user-manager'); ?></p>
						<p class="description"><?php esc_html_e('Debug: append ?um_text_file_line_count_debug=1 to the Admin: Orders URL to output URL-build/fetch/cache diagnostics for flagged file fields.', 'user-manager'); ?></p>
