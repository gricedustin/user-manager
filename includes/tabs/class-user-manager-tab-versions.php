				...
				<div class="um-admin-card-body">
					...
					<div class="um-changelog-item">
						...
						<ul>
							...
							<li><?php esc_html_e('Add-on settings include Page Title Override, Menu Title Override, Menu Location Priority, and a WP-Admin Bar Dropdown toggle.', 'user-manager'); ?></li>
							<li><?php esc_html_e('WP-Admin Bar dropdown lists the current user\'s favorite tiles, includes an "Add Current Page to Dashboard Tiles" quick action, and is pinned as an icon on mobile viewports so the shortcut stays reachable.', 'user-manager'); ?></li>
							<li><?php esc_html_e('Dashboard page has two tabs: Settings (sortable section + tile management with persistent ordering) and Dashboard (color-coded tile cards with click-count, last-clicked user/time, and a global search).', 'user-manager'); ?></li>
							<li><?php esc_html_e('JSON import/export rebuilds or migrates the tile set between sites; duplicates are skipped by id or by section title / tile title+URL.', 'user-manager'); ?></li>
						</ul>
					</div>
					<div class="um-changelog-item">
						<h4>2.6.14 <span>(April 18, 2026)</span></h4>
						<ul>
							<li><?php esc_html_e('Restricted Access add-on: replaced the 2.6.11 short-lived token fix with a fuller shared-password gate rewrite so visitors are always granted on the first submit.', 'user-manager'); ?></li>
							<li><?php esc_html_e('Password submissions are now processed before any redirect or overlay branch decision, so a correct password is never lost behind a cache/proxy or the background-HTML overlay path.', 'user-manager'); ?></li>
							<li><?php esc_html_e('Added a signed single-use grant token (stored in a transient) appended to the post-login redirect URL so access is still granted when a CDN or proxy drops the outbound Set-Cookie on a 3xx response. The grant param is consumed once and then stripped from the address bar.', 'user-manager'); ?></li>
							<li><?php esc_html_e('All post-login redirects now use HTTP 303 with explicit Cache-Control: no-store/no-cache/private, Pragma: no-cache, Vary: Cookie, and DONOTCACHEPAGE so they are never cached.', 'user-manager'); ?></li>
							<li><?php esc_html_e('The overlay password form now posts to dedicated admin-ajax.php / admin-post.php endpoints (action: um_restricted_access_submit) instead of the current page URL, so submissions work reliably even on cached/background-HTML pages.', 'user-manager'); ?></li>
							<li><?php esc_html_e('Added an inline JS handler that submits via fetch(), shows the server error in place, and navigates to the grant URL on success; a native form-POST fallback runs automatically on any JS/network error so users are never trapped.', 'user-manager'); ?></li>
							<li><?php esc_html_e('Removed the unverified wp_nonce_field from the password form (it added cache-invalidation surface without providing any security since it was never verified server-side).', 'user-manager'); ?></li>
						</ul>
					</div>
					<div class="um-changelog-item">
						<h4>2.6.13 <span>(April 18, 2026)</span></h4>
						<ul>
							<li><?php esc_html_e('My Account Admin Additional Meta Fields: added a Flexible Checkout Fields PRO File Upload flag ("fcf_file") that marks a row as an FCF PRO upload hash.', 'user-manager'); ?></li>
							<li><?php esc_html_e('When the flag is set, the plugin resolves the hash to a real file under wp-content/uploads/woocommerce_uploads/flexible-checkout-fields/<hash>/ before rendering links, previews, and line counts.', 'user-manager'); ?></li>
							...
							<li><?php esc_html_e('Additional Meta Fields repeater UI now includes a "Render as Flexible Checkout Fields PRO File Upload Field" checkbox alongside the existing Count/Preview/Show-when-empty flags.', 'user-manager'); ?></li>
					...
					<div class="um-changelog-item">
						<h4>2.6.6 <span>(April 18, 2026)</span></h4>
						<ul>
							<li><?php esc_html_e('Moved "Show Meta Data area for Order details" directly below "Default all new orders into a payment pending status" so related Order viewer toggles group together above the Additional Meta Fields repeaters.', 'user-manager'); ?></li>
						</ul>
					</div>
					<div class="um-changelog-item">
					...
					<div class="um-changelog-item">
						...
						<ul>
							...
							<li><?php esc_html_e('The export request is now intercepted at template_redirect (before WooCommerce/theme output starts), any buffered output is cleared, and only the CSV rows are streamed with proper CSV headers.', 'user-manager'); ?></li>
						</ul>
					</div>
					<div class="um-changelog-item">
						...
						<ul>
							...
							<li><?php esc_html_e('Consolidated pending work from the cart-and-order-page-settings and datalist-content-deferral branches into the main release line.', 'user-manager'); ?></li>
						</ul>
					</div>
					<div class="um-changelog-item">
