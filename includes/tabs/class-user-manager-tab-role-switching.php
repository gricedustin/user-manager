...
class User_Manager_Tab_Role_Switching {

	/**
	 * Render Role Switching tab.
	 *
	 * This mirrors the "View Website by Role" settings UI but inside User Manager.
	 */
	public static function render(): void {
		...
		// Settings for role switching (shared option key for compatibility).
		$settings     = get_option('view_website_by_role_settings', []);
		$enabled      = !empty($settings['enabled']);
		$hidden_roles = $settings['hidden_roles'] ?? [];
		$allow_reset  = !empty($settings['allow_reset']);

		// Handle settings save.
		if (isset($_POST['um_role_switching_settings_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['um_role_switching_settings_nonce'])), 'um_role_switching_settings')) {
			$new_enabled      = !empty($_POST['role_switching_enabled']);
			$new_hidden_roles = isset($_POST['hidden_roles']) ? array_map('sanitize_text_field', (array) wp_unslash($_POST['hidden_roles'])) : [];
			$new_allow_reset  = !empty($_POST['allow_reset']);

			$changes = [];
			if ($enabled !== $new_enabled) {
				$changes[] = $new_enabled ? 'Enabled Role Switching' : 'Disabled Role Switching';
			}
			$added_roles   = array_diff($new_hidden_roles, $hidden_roles);
			$removed_roles = array_diff($hidden_roles, $new_hidden_roles);
			if (!empty($added_roles)) {
				$changes[] = 'Hidden roles: ' . implode(', ', $added_roles);
			}
			if (!empty($removed_roles)) {
				$changes[] = 'Unhidden roles: ' . implode(', ', $removed_roles);
			}
			if ($allow_reset !== $new_allow_reset) {
				$changes[] = $new_allow_reset ? 'Enabled Reset Default Roles' : 'Disabled Reset Default Roles';
			}

			update_option('view_website_by_role_settings', [
				'enabled'      => $new_enabled,
			...
			if (!empty($changes)) {
				User_Manager_Core::add_role_switch_history('Settings Update', implode(' | ', $changes));
			}
			...
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Role Switching settings saved successfully.', 'user-manager') . '</p></div>';
		}
		...
		<div class="um-admin-grid">
			<div class="um-admin-card um-admin-card-full">
				<div class="um-admin-card-header">
					...
					<h2><?php esc_html_e('Role Switching Settings', 'user-manager'); ?></h2>
				</div>
				<div class="um-admin-card-body">
					<form method="post">
						<?php wp_nonce_field('um_role_switching_settings', 'um_role_switching_settings_nonce'); ?>
						<table class="form-table">
							<tr>
								...
								<td>
									<label>
										<input type="checkbox" name="role_switching_enabled" id="um-role-switching-enabled" value="1" <?php checked($enabled); ?> />
						...
						<?php submit_button(__('Save Role Switching Settings', 'user-manager')); ?>
					</form>
				</div>
			</div>
			...
			<div class="um-admin-card um-admin-card-full">
				<div class="um-admin-card-header">
					...
					<h2><?php esc_html_e('Users with Role Switching Access', 'user-manager'); ?></h2>
				</div>
				<div class="um-admin-card-body">
						...
						<tbody>
							...
							foreach ($users as $user) {
								...
								<tr>
									...
									<td>
										<span class="dashicons <?php echo isset($user_meta['active']) ? 'dashicons-yes' : 'dashicons-no'; ?>"
											style="color: <?php echo isset($user_meta['active']) ? '#46b450' : '#dc3232'; ?>;"></span>
									</td>
									<td>
										<span class="dashicons <?php echo isset($user_meta['admin']) ? 'dashicons-yes' : 'dashicons-no'; ?>"
											style="color: <?php echo isset($user_meta['admin']) ? '#46b450' : '#dc3232'; ?>;"></span>
									</td>
									<td>
			...
			<div class="um-admin-card um-admin-card-full">
				<div class="um-admin-card-header">
					<span class="dashicons dashicons-groups"></span>
					<h2><?php esc_html_e('Users Who Have Changed Their Role', 'user-manager'); ?></h2>
				</div>
				<div class="um-admin-card-body">
					<?php
					global $wpdb;
			...
			<div class="um-admin-card um-admin-card-full">
				<div class="um-admin-card-header">
					...
					<h2><?php esc_html_e('Role Switching Settings Change History', 'user-manager'); ?></h2>
				</div>
				<div class="um-admin-card-body">
					<?php
