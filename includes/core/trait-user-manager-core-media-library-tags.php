...
CSS;
	...
	private static function get_media_library_tag_videos_shortcode_style_html(): string {
		...
		return '<style>'
			...
			. '.um-media-library-tag-videos-wrap-multi{grid-template-columns:repeat(2,minmax(0,1fr));align-items:center;}'
			...
			. '.um-media-library-tag-video-item{display:flex;flex-direction:column;justify-content:center;min-width:0;}'
			...
			. '@media (max-width:782px){.um-media-library-tag-videos-wrap-multi,.um-media-library-tag-videos-wrap-multi.um-media-library-tag-videos-wrap-cols-3,.um-media-library-tag-videos-wrap-multi.um-media-library-tag-videos-wrap-cols-4{grid-template-columns:repeat(2,minmax(0,1fr));}.um-media-library-tag-videos-wrap-multi.um-media-library-tag-videos-wrap-cols-1{grid-template-columns:1fr;}.um-media-library-tag-videos-wrap-multi.um-media-library-tag-videos-wrap-cols-1 .um-media-library-tag-video-item{grid-column:1/-1;}}'
			. '</style>';
	}
...
JS;
	...
	public static function render_media_library_tags_gallery_block(array $attrs = []): string {
		...
		if ($album_description_position !== 'none') {
			...
			if ($group_links_html !== '' && !$has_single_video_result) {
				$album_tag_description_html = $group_links_html . $album_tag_description_html;
			}
			if ($has_single_video_result) {
				// Compact single-video mode renders its own 3-column row, so avoid
				// duplicating description/featured-image blocks above or below it.
				$album_tag_description_html = '';
			}
		}
		...
		>
			<?php if ($has_single_video_result && $group_links_html !== '') : ?>
				<?php echo $group_links_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php endif; ?>
			<?php if (!$has_single_video_result && $album_description_position === 'above' && $album_tag_description_html !== '') : ?>
			...
			<?php endif; ?>
			<?php if (!$has_single_video_result && $album_description_position === 'below' && $album_tag_description_html !== '') : ?>
...
@media (max-width: 782px) {
	...
	public static function get_media_library_tag_gallery_defaults(array $settings = []): array {
		...
		$single_video_three_column_setting_raw = null;
		if (isset($settings['media_library_tag_gallery_single_video_three_column_combined_row'])) {
			$single_video_three_column_setting_raw = $settings['media_library_tag_gallery_single_video_three_column_combined_row'];
		} elseif (isset($settings['media_library_tag_gallery_single_video_three_column_alternative_layout'])) {
			$single_video_three_column_setting_raw = $settings['media_library_tag_gallery_single_video_three_column_alternative_layout'];
		}
		if ($single_video_three_column_setting_raw !== null) {
			$defaults['singleVideoThreeColumnCompactLayout'] = (
				$single_video_three_column_setting_raw === true
				|| $single_video_three_column_setting_raw === '1'
				|| $single_video_three_column_setting_raw === 1
			);
		} else {
			$defaults['singleVideoThreeColumnCompactLayout'] = (bool) ($defaults['singleVideoThreeColumnCompactLayout'] ?? false);
		}
	...
	private static function render_media_library_tag_description_paragraphs_html(array $tag_description_data, array $layout_options = []): string {
		...
		if (!empty($layout_options['suppressFeaturedImage'])) {
